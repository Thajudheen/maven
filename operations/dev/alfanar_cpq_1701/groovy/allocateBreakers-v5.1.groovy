...
import static AssemblyValidaton;
	...
	class Constants {
		...
		static final boolean ENABLE_DIRECTPRINT = true;
		...
		//static final def GROUPING_ERPIDS = [INCOMING_ERPID, OUTGOING_ERPID] //to be deleted
	...
	class CubicleValidation{
		...
		private static SalesItemNode getSwitchBoard(SalesItemNode salesItemNode) {
			...
			def switchboardName = parentNode.getProductObjectName();
			
			if(SWITCHBOARD_OBJECTNAME.contains(switchboardName))
				return parentNode;
			else if(StringUtils.isBlank(switchboardName))
				return null;
			else
	...
	Closure <SalesItemNode> getCubicleByERPID = { SalesItemNode breaker, SalesItemNode switchboard, SalesItemNode parent, int balanceOfBreaker ->
		...
		def parentObjectName = parent.getProductObjectName();
		if(parentObjectName == null) {
			throw new Exception("getCubicleERPID --> parent '" + parent.getProductObjectName() + "'s name cannot be null. ");
		}
				...
				if(SBRatedCurrentMainBusBar <= 4000) {
					if (800 <= breakerRatedCurrent && breakerRatedCurrent <= 1600){
						...
						if (breakerRatedCurrent <= 250 && 
							...
							breakerNoOfPole == 3 &&
							balanceOfBreaker >= 3 &&
							parentObjectName == OUTGOING_ObjectName)
						{
	...
	Closure processBreakers = {SalesItemNode salesItemNode, SalesItemNode switchboard ->
		...
		String salesItemNodeObjectName = salesItemNode.getProductObjectName();
		...
		for (Map.Entry<String, List<SalesItemNode>> entry : categorizedBreakers.entrySet())
		{
			...
			if(salesItemNodeObjectName.contains(INCOMING_ObjectName) && materialCategory.contains(MaterialCategory.ACB.toString())) {
	...
	{
		groovyLogger.error(errorLog);
	}

	if (StringUtils.isNotBlank(infoLog))
	{
		groovyLogger.logInfo(infoLog);
	}
